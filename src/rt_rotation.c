...
/*   Updated: 2019/09/13 12:03:29 by khaniche         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_vec	rt_rotate_camera_x(t_camera *camera, t_vec ray_dir)
{
	...
	tmp = ray_dir[0];
	ray_dir[0] = tmp * cos(camera->orient[1]) + ray_dir[2] * sin(camera->orient[1]);
	ray_dir[2] = -tmp * sin(camera->orient[1]) + ray_dir[2] * cos(camera->orient[1]);
	return (ray_dir);
}
...
t_vec			rt_rotate_camera(t_camera *camera, t_vec ray_dir)
{
	...
	tmp = ray_dir[1];
	ray_dir[1] = tmp * cos(camera->orient[0]) + ray_dir[2] * sin(camera->orient[0]);
	ray_dir[2] = -tmp * sin(camera->orient[0]) + ray_dir[2] * cos(camera->orient[0]);
